...
from models import WordLevel, ExampleType, User, BatchSource, ContentType
from helpers import TextFormatter
from auth.repository import get_current_user
from words.tasks import process_bulk_words_task
from datetime import datetime, timezone
from schemas.words import WordCreate
from decorators import log_endpoint
import words.repository as WordRepository
import examples.repository as ExampleRepository
...
def get_words(
    ...
    logger.info(
        f"[get_words] User {current_user.id}: Fetching words (sort={sort}, page={page}, limit={limit})")
    paginated_data = WordRepository.get_words(
    ...
    paginated_data["items"] = [
        {
            ...
            "is_learned": all(stat.is_learned for stat in w.statistics if stat.type == ContentType.EXAMPLE) if w.statistics else False,
...
def get_word(
    ...
    word = WordRepository.get(db, word_id)
    ...
    is_learned = all(stat.is_learned for stat in word.statistics if stat.type ==
                     ContentType.EXAMPLE) if word.statistics else False
...
def create_single_word(
    ...
    try:
        ...
        if not text or not text.strip():
            raise HTTPException(
                status_code=400, detail="El campo 'text' es requerido")

        logger.info(
            f"[create_single_word] Processing word for user {current_user.id}: '{text}'")
        ...
        if not extracted_list or len(extracted_list) == 0:
            raise HTTPException(
                status_code=400, detail="No se pudo extraer una palabra válida del texto")

        extracted = extracted_list[0]
        main_word = extracted.get("main")
        if not main_word:
            raise HTTPException(
                status_code=400, detail="No se pudo determinar la palabra principal")
        ...
        if not enriched:
            raise HTTPException(
                status_code=400, detail="No se pudo enriquecer la palabra")

        logger.info(
            f"[create_single_word] Word enriched with: meaning, synonyms, level, category")

        manager = BatchManager(db)

        propitious_batch = manager.get_or_create_propitious_batch(
            user_id=current_user.id,
            source=BatchSource.ORGANIC
        )
        logger.info(
            f"[create_single_word] Assigned batch: {propitious_batch.id} ({propitious_batch.title})")
        ...
        # 5. Crear la palabra
        new_word = WordRepository.create(
            db, word_data, user_id=current_user.id)
        if not new_word:
            raise HTTPException(status_code=400, detail="La palabra ya existe")

        logger.info(
            f"[create_single_word] Word created: {new_word.id} - {new_word.main}")
        ...
        if enriched.get("examples"):
            ...
            if initial_examples:
                ...
                logger.info(
                    f"[create_single_word] Initial examples created: {len(raw_initial)}")
            ...
            if explore_examples:
                ...
                logger.info(
                    f"[create_single_word] Explore examples created: {len(raw_explore)}")
        ...
        logger.error(
            f"[create_single_word] Error creating word: {str(e)}", exc_info=True)
...
def toggle_word_boost(
    ...
    """
    logger.debug(
        f"[toggle_word_boost] User {current_user.id}: Toggling boost for word {word_id}")
    word = WordRepository.get(db, word_id)
    if not word or word.user_id != current_user.id:
        logger.warning(
            f"[toggle_word_boost] Word {word_id} not found or unauthorized")
        raise HTTPException(status_code=404, detail="Palabra no encontrada")
    ...
    db.commit()
    logger.info(
        f"[toggle_word_boost] Word {word_id} boost set to {word.is_boosted}")
...
def toggle_word_active(word_id: int, db: Session = Depends(get_db)):
    logger.debug(
        f"[toggle_word_active] Toggling active status for word {word_id}")
    word = WordRepository.toggle_active(db, word_id)
...
def toggle_word_learned(word_id: int, db: Session = Depends(get_db)):
    logger.debug(
        f"[toggle_word_learned] Toggling learned status for word {word_id}")
    word = WordRepository.toggle_learned(db, word_id)
    ...
    is_learned = all(stat.is_learned for stat in word.statistics if stat.type ==
                     ContentType.EXAMPLE) if word.statistics else False
    logger.info(
        f"[toggle_word_learned] Word {word_id} marked as {'learned' if is_learned else 'not learned'}")
    return {
        "id": word.id,
        "is_learned": is_learned,
...
def toggle_word_favorite(word_id: int, db: Session = Depends(get_db)):
    logger.debug(
        f"[toggle_word_favorite] Toggling favorite status for word {word_id}")
    word = WordRepository.toggle_favorite(db, word_id)
    ...
    logger.info(
        f"[toggle_word_favorite] Word {word_id} marked as {'favorited' if word.is_favorite else 'not favorited'}")
    return {
        "id": word.id,
        "is_favorite": word.is_favorite,
...
def export_words_csv(db: Session = Depends(get_db)):
    ...
    words_list = WordRepository.get_all_words(db)
